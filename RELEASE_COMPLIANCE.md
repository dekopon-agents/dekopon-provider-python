...
`dekopon-agents/dekopon-provider-python` releases. The accepted narrow packages are:
...
The owner acceptance is complete and carries forward to each release. The repository variable
...
`dekopon-python-provider-<version>-relink-source.tar.gz` from the immutable release commit,
where `<version>` is the version `Cargo.toml` declares at that commit. It contains:
...
Each GitHub Release deliberately contains exactly these 14 assets:

1. `python-provider.wasm`
2. `python-provider.wasm.sha256`
3. `dekopon-python-provider-<version>-relink-source.tar.gz`
4. `dekopon-python-provider-<version>-relink-source.tar.gz.sha256`
5. `dekopon-python-provider-<version>.cdx.json`
...
- `ghcr.io/dekopon-agents/provider-python:<version>`
...
`ghcr.io/dekopon-agents/provider-python-source:<version>`, including the source manifest digest. Both
...
- repository identity is exact, the variable interlock is true, `v<version>` is an annotated tag
  at the workflow SHA, package/tag versions match, the commit is contained in `main`, and no Wasm
  is tracked;
