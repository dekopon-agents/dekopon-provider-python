...
for stale in recover-v0.1.0.yml finalize-v0.1.0.yml release-recovery.yml; do
  if [[ -f "$root/.github/workflows/$stale" ]]; then
    echo "error: stale one-shot workflow was not deleted: $stale" >&2
    exit 1
  fi
done
