...
impl ToTokens for GetSetNursery {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let mut properties: Vec<_> = self.map.iter().collect();
        properties.sort_by(
            |((left_name, left_cfgs), _), ((right_name, right_cfgs), _)| {
                left_name.cmp(right_name).then_with(|| {
                    quote!(#(#left_cfgs)*)
                        .to_string()
                        .cmp(&quote!(#(#right_cfgs)*).to_string())
                })
            },
        );
        tokens.extend(
...
struct MemberNursery {
    map: BTreeMap<String, MemberNurseryEntry>,
    validated: bool,
}
